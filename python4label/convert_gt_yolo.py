...
ROOT_PATH = "/data1/workspace_hunter/mAP/"
IMG_PATH = os.path.join(ROOT_PATH, "images/head/")
GT_PATH = os.path.join(ROOT_PATH, "ground-truth/head_og/")
...
for tmp_file in txt_list:
  print(tmp_file)
  if tmp_file == "classes.txt": continue
  ...
  with open(tmp_file, "a") as new_f:
    for line in content:
      ...
      #print(obj_name + " " + str(left) + " " + str(top) + " " + str(right) + " " + str(bottom))
          new_f.write("face" + " " + str(left) + " " + str(top) + " " + str(right) + " " + str(bottom) + '\n')
