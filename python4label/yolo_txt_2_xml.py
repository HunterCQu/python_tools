...
def write_xml(width, hight, name, info, outxmlpath, filename):
    ...
    for i in range(len(name)):
        ...
        if w * h > box_area and str(name[i]) == LABEL:            #太小的框不要
            state = True
    if state == False:
        return False
...
picpath = "/home/anker/workspace_hunter/userdata/out/"   #原始图像路径
xmlpath = "/home/anker/workspace_hunter/userdata/labels/"   #原始xml路径
picnamelist = os.listdir(picpath)
xmlnamelist = os.listdir(xmlpath)
savepicpath = "/data1/workspace_hunter/tset_out/"   #生成的图像保存的路径
